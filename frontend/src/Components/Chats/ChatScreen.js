...
const ChatScreen = () => {
    ...
    useEffect(() => {
        ...
        const fetchAllMessage = async () => {
            const fetchedMessages = await axios.get("/messages/all/" + userId + "_" + location.state.matchUserId);
    ...
    return (
        <div className='chatScreen'>
            <p className='chatScreen__timestamp'>You Matched with {location.state.name} on {formatDayTime(location.state.matchTime)}</p>
            {messages.map(message => (

                message.fromId === location.state.matchUserId ? (<div key={message._id} className='chatScreen__message'>
                    <Avatar className='chatScreen__image'
                        alt={location.state.name}
                        src={location.state.image}
                        sx={{ bgcolor: deepOrange[500] }}
                    />
                    <p className='chatScreen__text'>{message.message}</p><small>{formatDayTime(message.timeStamp)}</small>
                </div>) : (<div id={message._id} className='chatScreen__messageUser'>
                    <p className='chatScreen__Usertext'>{message.message}</p><small>{formatDayTime(message.timeStamp)}</small>
                </div>)
